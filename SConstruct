...
'''
    ...
    install         Default target and copies it to build/mtchaind (default)
...
GCC 5: If the gcc toolchain is used, gcc version 5 or better is required. On
    linux distros that ship with gcc 4 (ubuntu < 15.10), mtchaind will force gcc
    to use gcc4's ABI (there was an ABI change between versions). This allows us
    to use the package manager to install mtchaind dependencies. It also means if
...
Clang on linux: Clang cannot use the new gcc 5 ABI (clang does not know about
    ...
    link mtchaind in this scenario (use the `--static` with scons), as dynamic
...
if (not platform.machine().endswith('64')):
    print('Warning: Detected {} architecture. Mtchaind requires a 64-bit OS.'.format(
          platform.machine()));
...
def check_openssl():
    ...
    if parse_time(d) < parse_time(build_time):
        raise Exception('Your openSSL was built on %s; '
                        'mtchaind needs a version built on or after %s.'
                        % (line, build_time))
...
for tu_style in ['classic', 'unity']:
    for toolchain in all_toolchains:
		...
		'src/slua/lsqlite3.c',
            ...
            target = env.Program(
                target=os.path.join(variant_dir, 'mtchaind'),
...
# Build the list of mtchaind source files that hold unit tests
