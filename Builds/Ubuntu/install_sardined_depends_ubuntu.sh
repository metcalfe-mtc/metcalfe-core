...
#
# This scripts installs the dependencies needed by mtchaind. It should be run
...
if [ ${ubuntu_release} == "14.04" ] || [ ${ubuntu_release} == "15.04" ]; then
    apt-get install python-software-properties
    echo "deb [arch=amd64] https://mirrors.mtchain.io/ubuntu/ trusty stable contrib" | sudo tee /etc/apt/sources.list.d/mtchain.list
    wget -O- -q https://mirrors.mtchain.io/mirrors.mtchain.io.gpg.key | sudo apt-key add -
    add-apt-repository ppa:ubuntu-toolchain-r/test
    apt-get update
    apt-get -y upgrade
